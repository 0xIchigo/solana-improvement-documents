...
| ------------------------------- | ------ |
| Transaction Execution (Runtime) | Epoch reward calculation only; `taper`/`initial` are re-anchored at the activation slot; no change to per-transaction execution |
| Virtual Machine                 | None |
| Block Packing                   | None |
| Consensus                       | Reward totals feed bank capitalization and the bank hash; all clients must compute identical values across the activation boundary |
| Gossip                          | None |
| Turbine                         | None |
...
| Other (please describe)         | RPC output (`getInflationRate`, `getInflationGovernor`) reflects the new schedule; `getInflationGovernor` returns the re-anchored `initial`. No API change. Adds feature `double_disinflation_rate` |
